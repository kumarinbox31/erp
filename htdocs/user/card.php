...
// Get Licence Data 
$licences = $pbAPI->licence();
$licences = $licences['data'];
$currentDate = date('Y-m-d');
$avlLicences = array_filter($licences, function ($license) use ($currentDate) {
    return $license['is_used'] == 0 && $license['status'] == 1 && ($license['expires_at'] > $currentDate or $license['expires_at'] == null);
});
$licenceCount = count($avlLicences);
...
if (empty($reshook)) {
	...
	if ($action == 'add' && $canadduser) {
		// write code to check licence to online
		$error = 0;
		if(empty($licenceCount)){
			$error++;
			setEventMessages($langs->trans("LicenceNotFound"), null, 'errors');
		...
		if (!$error) {
			...
			$db->begin();
			$firstavlLicence = reset($avlLicences);
			$licenceId = $firstavlLicence['license_id'];
			$licenceKey = $avlLicences['license_key'];
			$object->licence = $licenceKey;
			// echo '<pre>';print_r($object);exit;
			$id = $object->create($user);
			if ($id > 0) {
				$res = $pbAPI->useLicence($licenceId);
				$resPass = 0;
...
if ($action == 'create' || $action == 'adduserldap') {
	...
	print "<br>";
	print '<table class="pb-table pb-table-bordered noborder boxtable" >';
	print '<tr><th colspan="5">All Licences</th></tr>
	<tr><th>Key</th><th>Issue Date</th><th>Expire Date</th><th>Availability</th><th>Status</th></tr>
	<style>
		.td-center > td{
			text-align:center;
		}
	...
	foreach($licences as $licence){
		$used = $licence['is_used'] ? '<span style="color:red">Used</span>' : "<span style='color:green'>Available</span>";
			echo '<tr class="td-center">
					<td>'.$licence['license_key'].'</td>
					<td>'.$licence['issued_at'].'</td>
					<td>'.$licence['expires_at'].'</td>
					<td>'.$used.'</td>
					<td></td>
				</tr>';
	}
